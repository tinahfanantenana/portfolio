...
/**
 * Pour ajouter la navigation tactile pour la carousel
 */
...
class Carousel{
    ...
    constructor(element, options={}){
        ...
        if(this.#options.infinite&&this.#options.loop){
           throw new Error("la carousel ne peut être à la fois infinie et en boucle")
        }
        ...
        this.#carouselOnMobile();
        if (this.#options.infinite){
            this.offset=this.#slideVisible;
            if(this.offset>this.children){
                console.error("pas assez d'élémént por le carousel")
            }
        ...
        this.#onResize = this.#carouselOnMobile.bind(this);
    ...
    #createNavigation(){
        ...
        if (this.#options.loop==false){
            this.#onSlide(index=>{
                if(index==0){
                    prevButtom.classList.add('carousel_prev_hidden');
                }else{
                    prevButtom.classList.remove('carousel_prev_hidden');
                }
                if(index>=this.items.length || this.items[this.currentItem+this.#slideVisible]===undefined){
                    nextButtom.classList.add('carousel_next_hidden');
                }else{
                    nextButtom.classList.remove('carousel_next_hidden');
                }
            });
        }
    }
    ...
    #createPagination(){
        const pagination=this.#createDivWithClass('carousel_pagination');
        this.#root.append(pagination);
        const buttons=[];
        for( let i=0; i< (this.items.length - 2 * this.offset); i=i+this.#slideToScroll){
            const button=this.#createDivWithClass('carousel_pagination_button');
        ...
        this.#onSlide((index)=>{
            ...
            buttons.forEach((button,i)=>{
                if(i===activeButton){
                    button.classList.add('carousel_pagination_button--active');
                }else{
                    button.classList.remove('carousel_pagination_button--active');
                }
            })
        })
    }
